...
if ($response['lastpage'] > 1) {
    ...
    <div class='pages ui-buttonset'>
        <?php echo '<a class=\'ui-button ui-corner-left\' ' . ($response['pageno'] == 1 ? 'disabled' : 'bt') . ' ' . ($response['pageno'] == 1 ? '' : "href='{$url}/" . ($response['pageno'] == 2 ? '' : 'p/' . ($response['pageno'] - 1)) . '\'') . '>Anterior</a>';
    ...
    echo '<a class=\'ui-button ui-corner-right\' ' . ($response['pageno'] == $response['lastpage'] ? 'disabled' : '') . ' ' . ($response['pageno'] == $response['lastpage'] ? '' : "href='{$url}/p/" . ($response['pageno'] + 1) . '\'') . '>Próxima</a>'; ?>
    </div>
    <?php }